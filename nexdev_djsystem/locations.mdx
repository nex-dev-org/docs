...
description: "Four booths ship configured out of the box. Add your own in config/locations.lua, or build them in-game with /djcreate."
---
...
You can add booths two ways: edit `config/locations.lua`, or build them in-game with [`/djcreate`](/nexdev_djsystem/location-creator). The in-game route is easier and is what most people use — this page covers the config shape, which is also what the creator writes.
...
<Warning>
  With no prop there's no entity to target, so the anchor has to be somewhere reachable. If it's a metre inside geometry, the zone is unreachable. Nudge it with `/djedit`.
</Warning>
...
Booths added to `config/locations.lua` need a resource restart. Booths in the database can be reloaded live with `/djreload`, and edits made through `/djedit` push to everyone immediately.
