...
| `booth.spawn = false` | With no prop there's no entity to target, so a box zone is registered at the anchor instead. If the anchor is inside geometry, the zone is unreachable — nudge it with `/djedit`. |
...
| I edited a job-gated booth and now anyone can DJ | The editor can't pre-fill jobs, gangs, identifiers or the ACE principal, and **anything left blank is saved blank**. Re-enter them and save again. See [Location creator](/nexdev_djsystem/location-creator). |
...
6. Whether the booth came from `config/locations.lua` or `/djcreate`.
