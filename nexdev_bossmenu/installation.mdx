---
title: "Installation"
description: "How to install NEX.dev Boss Menu on your FiveM server."
---
...
1. Drop the `nexdev_bossmenu/` folder into your `resources/` tree, e.g.:
   ...
   ```
   resources/[assets]/nexdev_bossmenu
   ```
...
3. Add it to your `server.cfg`, **after** your framework core and after `ox_lib` / `oxmysql`:
   ...
   ensure nexdev_bossmenu
   ```

4. Edit `config.lua` as needed — see [Configuration](/nexdev_bossmenu/configuration).
...
```
[nexdev_bossmenu] Framework: qbx          -- detected core
[nexdev_bossmenu] Schema ready.           -- DB ready
[nexdev_bossmenu] Migrated `boss_menu_locations` -> `nexdev_bossmenu`.   -- if 3.x upgrade
```

The framework line is the most useful one — if it doesn't match the framework you actually run, set `Config.Framework` explicitly. See [Configuration → Framework](/nexdev_bossmenu/configuration#framework).

From your server console:

```
state nexdev_bossmenu
```
