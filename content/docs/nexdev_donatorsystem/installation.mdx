---
title: "Installation"
description: "How to install NEX.dev Donator System on your FiveM server."
---
...
| Any vehicle-keys script *(optional)* | Auto-detected — see [Configuration](/nexdev_donatorsystem/configuration#vehicle-keys). |
...
3. Start the server. The database tables create themselves on first boot. You should see:
   ...
   [nexdev_donatorsystem] Loaded 0 items, 4 categories, 0 deals.
   ```

That's the resource installed. Next, wire up your Tebex store — see [Tebex setup](/nexdev_donatorsystem/tebex-setup).
...
```
state nexdev_donatorsystem
```

Should print `started`. From F8 in-game:

```
lua print(GetResourceState('nexdev_donatorsystem'))
```
