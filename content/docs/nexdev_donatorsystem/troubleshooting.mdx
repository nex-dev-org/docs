...
description: "Most problems show themselves in the server console or the F8 client console — filter for [nexdev_donatorsystem] first."
---

Most problems show themselves in the **server console** or the **F8 client console** — filter for `[nexdev_donatorsystem]` first.
...
| `Tebex secret not configured (sv_tebexSecret convar)` | You haven't set the secret. Add `sv_tebexSecret "..."` to your `server.cfg` above `ensure nexdev_donatorsystem` and restart. See [Tebex setup](/nexdev_donatorsystem/tebex-setup#step-1-set-your-tebex-secret). |
...
| Order redeems for 0 currency | The package ID isn't in `ServerConfig.CoinPackages` and `ServerConfig.CoinPerDollar` is `0`. Either add the package to the map, or set a fallback rate. See [Tebex setup → Step 3](/nexdev_donatorsystem/tebex-setup#step-3-map-packages-to-currency). |
...
| Vehicle spawns but I have no keys | Either no keys script is detected, or you've set `Config.GiveVehicleKeys = false`. List of supported keys scripts is in [Configuration → Vehicle keys](/nexdev_donatorsystem/configuration#vehicle-keys). |
...
| Theme changes don't stick | They're saved per-server in the `nexdev_donatorsystem_settings` table. If they revert on restart, check the server console for SQL errors. |
...
```
[nexdev_donatorsystem][schema] 7/7 tables verified, 4 default categories seeded.
[nexdev_donatorsystem] Loaded N items, N categories, N deals.
```
...
```
nexdev_donatorsystem
nexdev_donatorsystem_items
nexdev_donatorsystem_categories
nexdev_donatorsystem_purchases
nexdev_donatorsystem_deals
nexdev_donatorsystem_codes
nexdev_donatorsystem_redeems
nexdev_donatorsystem_settings
```
...
1. Server console output filtered for `[nexdev_donatorsystem]`.
2. F8 client console output filtered for `[nexdev_donatorsystem]`.
