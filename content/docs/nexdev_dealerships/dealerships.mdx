...
Prefer a marker over a person? Add `Interaction = 'marker'` to the entry and the resource will draw an upward chevron at `PedCoordinates`. Tweak the marker style (type, scale, colour, range) in `Config.Marker` — see [Configuration](/nexdev_dealerships/configuration#marker-when-a-lot-uses-interaction-marker).
...
The in-game option is easier and survives updates — see [Management dashboard](/nexdev_dealerships/management#display-vehicles).
