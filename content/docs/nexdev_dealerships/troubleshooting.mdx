...
Check the **F8** console (client) and your **server console**, filtered for `[nexdev_dealerships]`. Most issues print a clear error there.
...
| Lots don't show up after editing `shared/locations.lua` | You restarted the wrong resource. Use `restart nexdev_dealerships` (or `refresh ; ensure nexdev_dealerships`). Make sure each entry's **key** (`['pdm']`) is unique. |
| Vehicle is bought but doesn't show in the player's garage | Wrong garage script wiring. The resource writes to `owned_vehicles` (ESX) or `player_vehicles` (QB/Qbox) — your garage script must read from that. If you use a non-standard schema, set `Config.GiveVehicleFn`. See [Integrations → Garage](/nexdev_dealerships/integrations#garage-persistence). |
...
| `/dealershipadmin` does nothing when typed | You don't have permission. Add it with `add_ace identifier.steam:<HEX> nexdev_dealerships.admin allow`, or grant `group.admin` via `add_principal`. |
...
| Financing payments don't auto-debit | Players need to be online (or recently online) for installments to fire. If `Config.Financing.Enabled = true` and a real framework is detected, the worker runs every minute. Check the console for `[nexdev_dealerships]` errors. |
...
| Owner cut doesn't pay out | Owner identifier in `nexdev_dealership_settings.owner_identifier` is wrong or unset. Re-run `/dealershipadmin` → **Set Owner** with the player's steam identifier visible while they're online. |
...
3. Boot log with `Config.Debug = true`, filtered for `[nexdev_dealerships]`.
4. Server console output when the issue happens, filtered for `[nexdev_dealerships]`.
