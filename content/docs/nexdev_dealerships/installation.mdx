---
title: "Installation"
description: "How to install NEX.dev Dealerships on your FiveM server."
---
...
Everything else is optional and auto-detected — see [Optional integrations](/nexdev_dealerships/integrations).

## Steps

1. Drop the resource into your server, e.g. `resources/[nex]/nexdev_dealerships/`.
...
4. Grant admin access for the catalog panel. Either:
   ...
   ```
   add_ace group.admin nexdev_dealerships.admin allow
   ```
...
The resource creates all `nexdev_dealership_*` tables on boot. If you'd rather pre-create them — to inspect the schema, split maintenance windows, or pre-grant DB permissions — run the matching file from `install/`:
...
```
lua print(GetResourceState('nexdev_dealerships'))
```
