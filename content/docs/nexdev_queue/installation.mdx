---
title: "Installation"
description: "NEX.dev Queue is a server-only, drag-and-drop resource. There is no client UI to build and no database to migrate."
---

NEX.dev Queue is a **server-only, drag-and-drop** resource. There is no client UI to build and no database to migrate.
...
There is **no framework requirement**. NEX.dev Queue works on QBCore, Qbox, ESX, Ox Core, or a bare-bones FiveM server — it runs entirely server-side and does not touch your framework.

## Step 1 — Drop the resource in [#step-1-drop-the-resource-in]
...
## Step 2 — Add it to server.cfg [#step-2-add-it-to-server-cfg]
...
## Step 3 — Set up your Discord bot [#step-3-set-up-your-discord-bot]
...
## Step 4 — Fill in `config.lua` [#step-4-fill-in-config-lua]
