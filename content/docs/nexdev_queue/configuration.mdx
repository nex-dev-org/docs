---
title: "Configuration"
description: "Everything lives in nexdev_queue/config.lua. It's split into five sections:"
---
...
<Warning>
  You **do not** set the player cap here. NEX.dev Queue reads `sv_maxclients` directly from your `server.cfg`. `ReservedSlots` only carves slots off the top of that number for staff bypass.
</Warning>
...
One of: `classic`, `banner`, `factset`, `boardingpass`, `compactbar`, `minimal`, `centered`, `cards`. See [Themes](/nexdev_queue/themes) for previews.
...
Flip `Enabled = true` to refuse anyone who doesn't hold one of `Roles`. Bypass roles still get in. See [Whitelist & priority](/nexdev_queue/whitelist-and-priority) for details.
...
Full breakdown in [Whitelist & priority](/nexdev_queue/whitelist-and-priority).
...
| Change | Needs `restart nexdev_queue`? |
| ------ | ---------------------- |
| Anything in `Config.Server` | Yes |
| `Config.Interface.Theme` | No — see [Themes — hot-swap](/nexdev_queue/themes#hot-swap-at-runtime) |
...
You can read queue state from your other scripts. All exports are namespaced to `nexdev_queue`.

| Export | Returns |
| ------ | ------- |
| `exports.nexdev_queue:GetNexQueueCount()` | Total players currently in queue |
| `exports.nexdev_queue:GetNexQueueFullList()` | Sorted list of waiters (highest priority first) |
| `exports.nexdev_queue:GetPlayerNexStatus(src)` | Per-player snapshot (priority, tier, position, etc.) |
| `exports.nexdev_queue:GetNexThemeList()` | List of every valid theme name |
| `exports.nexdev_queue:GetNexActiveTheme()` | The currently active theme |
| `exports.nexdev_queue:SetNexActiveTheme("themeName")` | Swap theme without a restart |
| `exports.nexdev_queue:GetDiscordName(src)` | The connecting player's Discord username |
| `exports.nexdev_queue:GetDiscordNickname(src)` | Their server nickname |
| `exports.nexdev_queue:GetDiscordAvatar(src)` | URL to their avatar |
| `exports.nexdev_queue:GetDiscordRoles(src)` | Array of role IDs they hold |
| `exports.nexdev_queue:GetGuildName()` | Your guild name |
| `exports.nexdev_queue:GetGuildMemberCount()` | Approximate member count |
| `exports.nexdev_queue:GetGuildOnlineMemberCount()` | Approximate online count |
| `exports.nexdev_queue:GetGuildIcon()` | URL to your guild icon |
| `exports.nexdev_queue:GetGuildRoleList()` | The full role list, cached from Discord |
