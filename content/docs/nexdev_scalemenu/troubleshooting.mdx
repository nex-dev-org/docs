...
description: "Common nexdev_scalemenu issues and fixes — oxmysql order, the database table, permissions, Discord roles, gender lock, and persistence."
---
...
- Confirm the resource started cleanly in the server console — look for the `nexdev_scalemenu` startup banner and no Lua errors.
- If permissions are enabled, make sure the player has the `openSelf` action. With the system enabled, the command silently does nothing for players who lack permission. See [Permissions](/nexdev_scalemenu/permissions).
...
<Callout type="warn">
  ...
  `oxmysql` must be started **before** `nexdev_scalemenu`. Put `ensure oxmysql` first in `server.cfg`, before your framework and before this resource.
...
The `nexdev_scalemenu` table is created automatically on first start with `CREATE TABLE IF NOT EXISTS`. If it's missing:

- Make sure `oxmysql` started **before** `nexdev_scalemenu` (see above) — the schema can't be created otherwise.
...
- To give admins `openOther` / `resetOther`, add the ACE line and assign the group:
  ...
  ```cfg
  add_ace group.admin nexdev_scalemenu.admin allow
  ```

- See [Permissions](/nexdev_scalemenu/permissions) for the full layered model.
