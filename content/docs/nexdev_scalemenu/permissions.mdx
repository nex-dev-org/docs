---
title: "Permissions"
description: "The layered permission stack for nexdev_scalemenu — defaultPermissions, ACE, identifier whitelist, and Discord roles."
---

`nexdev_scalemenu` has a tiered permission system. It is **disabled by default**, so out of the box everyone can open and reset their own scale. When you enable it, access is resolved through four layers in order.
...
<Callout type="warn">
  ...
  If the bot token, guild ID, or the **Server Members Intent** is missing or wrong, the Discord lookup returns no roles and the Discord layer simply grants nothing. The other layers still apply. See [Troubleshooting](/nexdev_scalemenu/troubleshooting).
