---
title: "Installation"
description: "Drag-and-drop install for nexdev_scalemenu — single folder, correct server.cfg order, and an auto-created MySQL table."
---
...
3. (Optional) Open `config.lua` and adjust ranges, presets, permissions, or UI tokens. See [Configuration](/nexdev_scalemenu/configuration).
...
```sql
CREATE TABLE IF NOT EXISTS `nexdev_scalemenu` (
...
- If the menu does not open or scales do not persist, see [Troubleshooting](/nexdev_scalemenu/troubleshooting).
