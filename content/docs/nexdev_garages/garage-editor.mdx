...
<Callout type="info">
  ...
  Custom garages are stored in the database (`nexdev_custom_garages`), so they
...
Set the `nexdev_garages_webhook_custom_garages` convar to log every editor
action to Discord — see
[Configuration → Discord webhooks](/nexdev_garages/configuration#discord-webhooks-server-cfg).
