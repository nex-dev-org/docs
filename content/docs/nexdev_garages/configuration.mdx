---
title: "Configuration"
description: "Where every NEX.dev Garages setting lives and what the important toggles do."
---
...
|---|---|
| `config/main.lua` | Locale, currency/date format, framework selector, UI accent color, cinematic mode, SQL auto-install, debug |
| `config/public-garages.lua` | Public garage locations |
| `config/job-garages.lua` | Job garage locations & spawner fleets |
| `config/gang-garages.lua` | Gang garage locations |
| `config/impound-locations.lua` | Impound lot locations |
| `config/interior.lua` | The walk-in showroom interior |
| `config/garage-defaults.lua` | Shared blip & marker defaults for the location files |
| `config/custom-garages.lua` | The in-game garage editor (`/garagesadmin`) + staff ACE |
| `config/private-garages.lua` | Player-owned private garages |
| `config/impound.lua` | Impound command, society fund, hold durations, fee cap, proximity rule |
...
`hideHud` auto-detects common HUD resources (`qbx_hud`, `qb-hud`, `ps-hud`,
`jg-hud`, `izzy-hud`, `0r-hud`) — pick one explicitly or turn it off with the
`hud` option in `config/integrations.lua`.
...
## Discord webhooks (server.cfg) [#discord-webhooks-server-cfg]
...
```cfg
set nexdev_garages_webhook_store_takeout   "https://discord.com/api/webhooks/..."
set nexdev_garages_webhook_player_transfer "https://discord.com/api/webhooks/..."
set nexdev_garages_webhook_garage_transfer "https://discord.com/api/webhooks/..."
set nexdev_garages_webhook_impound         "https://discord.com/api/webhooks/..."
set nexdev_garages_webhook_private_garages "https://discord.com/api/webhooks/..."
set nexdev_garages_webhook_custom_garages  "https://discord.com/api/webhooks/..."
```
