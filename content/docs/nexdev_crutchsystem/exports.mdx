...
```lua
exports.nexdev_crutchsystem:ForceCrutch(playerId, 600, 'EMS')
```
...
```lua
if exports.nexdev_crutchsystem:IsPlayerCrutched(source) then
...
```lua
exports.nexdev_crutchsystem:SetWalkStyle('move_m@drunk@verydrunk')
```
...
| --- | --- | --- |
| `nexdev_crutchsystem:client:crutch` | — | toggle from inventory use |
| `nexdev_crutchsystem:client:forceCrutch` | `duration?` | `ForceCrutch` |
| `nexdev_crutchsystem:client:clearForce` | — | `ClearForce` |
| `nexdev_crutchsystem:client:forceRemove` | — | `RemoveCrutch` |
