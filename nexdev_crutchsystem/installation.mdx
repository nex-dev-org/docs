---
title: "Installation"
description: "How to install NEX.dev Crutch System on your FiveM server."
---
...
1. Drop the `nexdev_crutchsystem/` folder into your `resources/` tree, e.g.:
   ...
   ```
   resources/[assets]/nexdev_crutchsystem
   ```
...
['crutch'] = {
    ...
    server = {
        export = 'nexdev_crutchsystem.useCrutch',
    },
},
```
...
QBCore.Functions.CreateUseableItem('crutch', function(source, item)
    exports['nexdev_crutchsystem']:useCrutch(source, item, item.slot, nil)
end)
```
...
ESX.RegisterUsableItem('crutch', function(source)
    ...
    exports['nexdev_crutchsystem']:useCrutch(source, item, item and item.slot, nil)
end)
```
...
```
state nexdev_crutchsystem
```

Should print `started`. From F8 in-game:

```
lua print(GetResourceState('nexdev_crutchsystem'))
```
