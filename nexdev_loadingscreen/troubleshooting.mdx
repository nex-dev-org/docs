...
description: "Fixes for common NEX.dev Loading Screen issues."
---
...
| Loading screen is blank / no UI | Resource didn't start. From F8: `lua print(GetResourceState('nexdev_loadingscreen'))` should print `started`. If not, fix `server.cfg` and restart. Also check the server console for boot errors. |
| Player count / uptime never updates from the placeholder | The stats panel is fed by `nexdev_loadingscreen:stats` from `server.lua`. Confirm the resource is started server-side and `Config.Stats.enabled = true`. The first push can take up to `Config.Stats.poll` ms (default 5s). |
...
2. F8 console output filtered for `[nexdev_loadingscreen]` (or any errors mentioning it).
